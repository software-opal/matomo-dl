...
def main():
    ...
    version_file = REPO_ROOT / "matomo_dl/__version__.py"
    old_version_content = version_file.read_text()
    try:
        with (REPO_ROOT / "matomo_dl/__version__.py").open("w") as version_file:
            version_file.write(
                f"""\
    # Autogenerated from {sha}
    __version__ = "{tag}"
    """
            )
        ...
        version_file.write_text(old_version_content)
