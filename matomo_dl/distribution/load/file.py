...
def parse_license_key(base_dir: pathlib.Path, key: typ.Optional[str]) -> typ.Optional[str]:
    if key is None:
        return None
    key = key.strip()
    if not key:
...
def parse_plugins(plugins: typ.Optional[typ.Mapping[str, typ.Any]]) -> typ.Collection[dist_file.Plugin]:
    if not plugins:
        return []
    dat = []
    for name, value in plugins.items():
        if isinstance(value, str):
            ...
            dat.append(dist_file.GitPlugin(
                name, value['git'], value.get('ref') or 'master'))
        elif value.get('link'):
            dat.append(dist_file.RawPlugin(name, value['link']))
        else:
            dat.append(dist_file.VersionedPlugin(
                name, dist_version.AnyVersion()))
    return dat


def parse_customisations(customisations: typ.Optional[typ.Mapping[str, typ.Any]]) -> dist_file.Customisation:
...
def load_distribution_file(base_dir: pathlib.Path, file_content: str) -> dist_file.DistributionFile:
